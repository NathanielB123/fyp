...
-- Agda's termination checker is not super happy with these definitions
-- If we have time at some point, we should probably define an explicit 
-- elimination principle and use that, but in the meantime, I will just use
-- some pragmas
--
-- A likely culprit for the termination failures appears to be the calls to
-- |funext| - specifically the types involved at these |funext| clauses
-- break termination if they are computed explicitly. If we worked in a theory
-- with built-in |funext|, I believe this pain would be avoided.
...
⟦ ,rwℱ {b = true} δ refl refl t~ ⟧ TMS 
  = λ ρ → ⟦ δ ⟧Tms ρ Σ, box (Tm≡-inst ⟦ t~ ⟧Tm~)
⟦ ,rwℱ {b = false} δ refl refl t~ ⟧ TMS 
  = λ ρ → ⟦ δ ⟧Tms ρ Σ, box (Tm≡-inst ⟦ t~ ⟧Tm~)
⟦ δ ⨾ σ ⟧ TMS = λ ρ → ⟦ δ ⟧Tms (⟦ σ ⟧Tms ρ)
  
⟦𝔹⟧′ = refl

⟦_⟧>rw : Γ ⊢ t >rw b → ⟦ t ⟧Tm ≡ᴾ λ _ → b
...
⟦ rz     ⟧>rw = funextᴾ (λ ρ → ρ .snd .unbox)
⟦ rs p   ⟧>rw = funextᴾ (λ ρ → Tm≡-inst ⟦ p ⟧>rw) 
⟦ rsrw p ⟧>rw = funextᴾ (λ ρ → Tm≡-inst ⟦ p ⟧>rw) 
